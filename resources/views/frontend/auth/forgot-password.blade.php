...
@section('content')
<p class="h4 fw-semibold mb-2">{{ __('label.forgot_password') }}</p>
<p class="mb-3 text-muted op-7 fw-normal">{{ __('string.please_fill_your_phone_number') }}</p>

<form id="form-register" method="post" action="{{ route('auth.store.forgot-password') }}" class="form-block mb-4">
    @csrf
    ...
    <div class="mt-3">
        ...
        <div class="d-grid mt-4">
            <button type="button" class="btn btn-lg btn-primary btn-submit w-100" data-loading="{{ strtoupper(__('label.please_wait')) }}...">
    ...
    <div class="mt-4">
        <a href="{{ route('base') }}" class="text-primary fw-bold">
            <i class="fa-solid fa-arrow-left"></i> &nbsp;{{ __('label.login') }}
        </a>
    </div>
</form>
@endsection
