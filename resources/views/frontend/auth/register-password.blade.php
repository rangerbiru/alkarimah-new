...
@section('content')
<p class="h4 fw-semibold mb-2">{{ __('label.account_password') }}</p>
<p class="mb-3 text-muted op-7 fw-normal">{{ __('string.please_fill_your_password') }}</p>

<form id="form-register" method="post" action="{{ route('auth.store.register-password', $parent->encrypted_id) }}" class="form-block mb-4">
    @csrf
