...
<body>
    @php
        ...
        $set_bt_home = $controller == 'dashboard' ? ' active' : '';
        $set_bt_payment = $controller == 'finance' && $action == 'payment' ? ' active' : '';
        $set_bt_savings = $controller == 'finance' && $action == 'savings' ? ' active' : '';
        $set_bt_service = $controller == 'service' && $action == 'dashboard' ? ' active' : '';
        $set_bt_dakwah = Route::is('dakwah.index') ? ' active' : '';
        $set_bt_product = Route::is('product.index') ? ' active' : '';
    @endphp
    ...
    <div class="page">
        ...
        @else
            <div class="d-sm-flex d-block align-items-center justify-content-between page-header-breadcrumb"
                style="min-height: auto;margin-bottom: -45px;"></div>
        @endif
    ...
    <nav class="navbar navbar-bottom navbar-expand d-md-none d-lg-none d-xl-none">
        <ul class="navbar-nav nav-justified w-100">
            <li class="nav-item">
                <a class="nav-link{{ $set_bt_home }}" href="{{ route('dashboard.index') }}">
            ...
            @if (Auth::user()->hasRole('pegawai'))
                <li class="nav-item">
                    <a class="nav-link{{ $set_bt_dakwah }}" href="#" onclick="maintenance()">
                        ...
                        <div class="label">{{ __('label.dakwah') }}</div>
                    </a>
                </li>
                <li class="nav-item">
                    <a class="nav-link{{ $set_bt_product }}" href="#" onclick="maintenance()">
                        <i class="fa-solid fa-cube"></i>
                        <div class="label">{{ __('label.produk') }}</div>
                    </a>
                </li>
                <li class="nav-item">
                    <a class="nav-link{{ $set_bt_service }}" href="#" onclick="maintenance()"
                        data-toggle="offcanvas">
                        <i class="fa-solid fa-users"></i>
                        <div class="label">{{ __('label.kepegawaian') }}</div>
                    </a>
                </li>
            @else
                <li class="nav-item">
                    <a class="nav-link{{ $set_bt_payment }}" href="{{ route('finance.payment.index') }}">
                        <i class="ti ti-credit-card-filled"></i>
                        <div class="label">{{ __('label.payment') }}</div>
                    </a>
                </li>
                <li class="nav-item">
                    <a class="nav-link{{ $set_bt_savings }}" href="{{ route('finance.savings.index') }}">
                        <i class="ti ti-cash-banknote-filled"></i>
                        <div class="label">{{ __('label.savings') }}</div>
                    </a>
                </li>
                <li class="nav-item">
                    <a class="nav-link{{ $set_bt_service }}" href="{{ route('service.dashboard.index') }}"
                        data-toggle="offcanvas">
                        <i class="ti ti-layout-grid-filled"></i>
                        <div class="label">{{ __('label.service') }}</div>
                    </a>
                </li>
            @endif
