...
<div class="header-content-right">
    @foreach ($data->header as $menu)
        ...
        if (empty($menu->route))
            ...
            $route = (empty($menu->route->params)) ? route($menu->route->name) : route($menu->route->name, (array) $menu->route->params);
        }
        @endphp
        ...
        @else
            <div class="header-element">
                <a href="javascript:void(0);" class="header-link header-link-icon-name dropdown-toggle" data-bs-toggle="dropdown" data-bs-auto-close="outside">
                ...
                <ul class="main-header-dropdown dropdown-menu border-0" data-popper-placement="none">
                    @foreach ($menu->child as $c)
                        ...
                        <li>
                            <a class="dropdown-item d-flex align-items-center{{ $menu_active }}" href="{{ $route }}">
                                {{ $c->name }}
                            </a>
                        </li>
                    @endforeach
    ...
    <div class="header-element mainuserProfile">
            ...
            <div class="d-flex align-items-center">
                <div class="d-sm-flex wd-100p">
                    <div class="avatar avatar-sm">
                        <img alt="avatar" class="rounded-circle" src="{{ Auth::user()->photo }}" style="border: 2px solid white;" />
                    </div>
        ...
        <ul class="dropdown-menu  border-0 main-header-dropdown  overflow-hidden header-profile-dropdown" aria-labelledby="mainHeaderProfile">
            <li><a class="dropdown-item border-bottom" href={{ route('user.profile') }}><i class="fs-13 me-2 bx bx-user"></i>Profile</a></li>
            <li><a class="dropdown-item border-bottom" href={{ route('user.password') }}><i class="fs-13 me-2 bx bx-lock"></i>Change Password</a></li>
