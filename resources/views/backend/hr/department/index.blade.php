...
@section('content')
    <div class="card">
        <div class="card-body">
            <x-section-page-action :label="$title" :icon="$icon" :count="$count" :create-route="route('hr.department.create')" />
    ...
    <script>
        $(document).ready(function() {
            ...
            window.LaravelDataTables["table-position"] = $("#table-position").DataTable({
                ...
                ajax: {
                    url: "{{ route('hr.department.datatable') }}",
                    type: "POST"
                },
                processing: true,
                serverSide: true,
                ...
                columns: [{
                    ...
                    {
                        class: "align-middle",
                        render: (data, type, row, meta) => htmlEntities(row.position.name)
                    },
                    {
                        class: "align-middle text-center",
                        searchable: false,
                        render: function(data, type, row) {
                            let url_destroy = "{{ route('hr.department.destroy', ':id') }}"
                            url_destroy = url_destroy.replace(":id", row.id)

                            return `
                        <button class="btn btn-danger btn-xs set-tooltip fs-12" title="Hapus" onclick="deleteDepartment(${row.id}, '${url_destroy}')">
                            Hapus
                        </button>`
                        }
                    }
        ...
        function deleteDepartment(id, url_destroy) {
            Swal.fire({
