...
@section('title', $title)
@section('header')
    <x-section-page :label="$title" :icon="$icon" breadcrumb="hr/position" />
@endsection

@section('content')
    <div class="card">
        <div class="card-body">
            <x-section-page-action :label="$title" :icon="$icon" :count="$count" :create-route="route('hr.position.create')" />
    ...
    <script>
        $(document).ready(function() {
            ...
            window.LaravelDataTables["table-position"] = $("#table-position").DataTable({
                ...
                ajax: {
                    url: "{{ route('hr.position.datatable') }}",
                    type: "POST"
                },
                processing: true,
                serverSide: true,
                ...
                columns: [{
                    ...
                    },
                    {
                        ...
                        render: function(data, type, row) {
                            let url_edit = "{{ route('hr.position.edit', 0) }}"
                            let url_destroy = "{{ route('hr.position.destroy', 0) }}"
