    ...
    <div class="card card-tab mb-3">
        <div class="card-body p-0">
            <div class="bg-light p-3">
                {!! __('string.please_make_payment', [
                    'date' => Common::dateFormat($transaction->expired_view_at, 'dd mmm yyyy Pukul hh:ii WIB'),
                ]) !!} :
            </div>
            ...
            <div class="p-3">
                <div class="d-flex">
                    <div>
                        <small class="fw-bold text-grey">{{ __('label.transaction_number') }} :
                            {{ $transaction->number }}</small>
                        <div class="d-flex align-items-center gap-1">
                            <h3 class="text-success mb-0 mt-1">Rp. {{ number_format($transaction->total, 0, '', '.') }}</h3>
                            <a href="javascript:void(0)" id="btn-copy" class="text-grey fs-20"
                                data-clipboard-text="{{ $transaction->total }}"><i class="ti ti-copy"></i></a>
                        </div>
                    </div>
                    <div class="ms-auto">
                ...
                <div class="mb-3">
                    ...
                    <h5 class="text-grey">
                        ...
                        <a href="javascript:void(0)" id="btn-copy" class="text-grey"
                            data-clipboard-text="{{ $transaction->bank_account->number }}"><i class="ti ti-copy"></i></a>
                    </h5>
                </div>
                <div class="mb-3">
    ...
    <div class="card card-tab mb-3">
        <div class="card-body p-3">
            <div class="d-flex mb-2">
                <div class="fw-bold"><i class="ti ti-receipt-filled text-primary"></i> {{ __('label.transaction_detail') }}
                </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            clipboard.on("success", function(e) {
                $(e.trigger).html(
                    "<i class='ti ti-copy'></i> <small class='fw-normal' style='font-size: 13px;'>Copied</small>"
                )
            ...
            $("#btn-check").click(function() {
                ...
                btn.addClass("btn-loader").html(
                    `<span class="loading"><i class="ri-refresh-line fs-16"></i></span> &nbsp;&nbsp;{{ __('label.checking') }}`
                ).attr("disabled", true)
                ...
                Swal.fire({
                    ...
                    preConfirm: () => {
                        $.ajax({
                            ...
                            dataType: "json",
                            success: function(response) {
                                setNotifSuccess(response.message, response.data
                                    .redirect)
                            },
                            error: function(xhr, ajaxOptions, thrownError) {
                                ajaxError(xhr.status)
