...
<div class="card custom-card">
    <div class="card-body">
        <x-section-page-action
            ...
            :icon="$icon"
            :count="$count"
            :create-route="route('finance.bill.type.create')"
        />
...
<script>
const period_monthly = "{{ $period_monthly }}"
...
$(document).ready(function() {
    ...
    window.LaravelDataTables["table-type"] = $("#table-type").DataTable({
        ...
        ajax:
        {
            url: "{{ route('finance.bill.type.datatable') }}",
            type: "POST"
        },
        processing: true,
        serverSide: true,
        ...
        columns: [
            ...
            {
                class: "align-middle",
                render: (data, type, row, meta) => row.period_name
            },
            {
                class: "align-middle text-center",
                searchable: false,
                render: function(data, type, row) {
                    let url_edit = "{{ route('finance.bill.type.edit', 0) }}"
                    let url_destroy = "{{ route('finance.bill.type.destroy', 0) }}"
