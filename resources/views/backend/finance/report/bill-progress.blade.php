...
@section('title', $title)
@section('header')
    <x-section-page :label="$title" :icon="$icon" breadcrumb="finance/report/bill-progress" />
@endsection
            ...
            <form class="form-block">
                <div class="row">
                    <div class="col-sm-6 col-md-3">
                        <x-form.select name="year" :option="$years" :data-placeholder="__('label.choose_school_year')" :old="$year" />
                    </div>
                    <div class="col-sm-6 col-md-5">
                        <button type="button" class="btn btn-secondary btn-submit"
                            data-loading="{{ strtoupper(__('label.searching')) }}">
    ...
    <div class="card custom-card">
        <div class="card-body">
            <div id="button-download" class="mb-3">
                <a href="{{ route('finance.report.download.excel.bill-progress', ['year' => $year]) }}"
                    class="btn btn-success label-btn">
                    <i class="fa-solid fa-file-excel label-btn-icon me-2"></i>
                    DOWNLOAD EXCEL
                </a>
                <a href="{{ route('finance.report.download.pdf.bill-progress', ['year' => $year]) }}"
                ...
                <table class="table">
                        ...
                        @foreach ($bill_progress as $index => $progress)
                            <tr>
                                ...
                                <td class="fw-bold" colspan="6">
                                    {{ __('label.level_class') . ' ' . $progress['level_name'] }}</td>
                            </tr>
                                ...
                                <tr>
                                    ...
                                    <td style="width: 100px;">
                                        <div class="progress" role="progressbar" aria-label="Animated striped example"
                                            ...
                                            <div class="progress-bar {{ $p->progress_color }} progress-bar-striped progress-bar-animated"
                            ...
                            <tr>
                                <td></td>
                                <td><b>{{ __('label.total') . ' ' . __('label.level_class') . ' ' . $progress['level_name'] }}</b>
                                </td>
                                <td style="text-align: right;"><b>{{ number_format($progress['total'], 0, '', '.') }}</b>
                                </td>
                                <td style="text-align: right;"><b>{{ number_format($progress['paid'], 0, '', '.') }}</b>
                                </td>
                                <td style="text-align: right;">
                                    <b>{{ number_format($progress['remaining'], 0, '', '.') }}</b>
                                </td>
                                <td style="width: 100px;">
                                    <div class="progress" role="progressbar" aria-label="Animated striped example"
                                        aria-valuenow="{{ $progress['progress'] }}" aria-valuemin="0" aria-valuemax="100">
                                        <div class="progress-bar {{ $progress['progress_color'] }} progress-bar-striped progress-bar-animated"
                    ...
                    <tfoot>
                        <tr>
                            ...
                            <td style="width: 100px;">
                                <div class="progress" role="progressbar" aria-label="Animated striped example"
                                    aria-valuenow="{{ $sum->progress }}" aria-valuemin="0" aria-valuemax="100">
                                    <div class="progress-bar {{ $sum->progress_color }} progress-bar-striped progress-bar-animated"
                                        style="width: {{ $sum->progress }}%"></div>
                                </div>
                            </td>
                            <td style="width: 80px;">
