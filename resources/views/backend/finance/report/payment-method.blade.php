...
@section('title', $title)
@section('header')
    <x-section-page :label="$title" :icon="$icon" breadcrumb="finance/report/payment-method" />
@endsection
            ...
            <form method="get" class="form-block">
                <div class="row">
                    ...
                    <div class="col-sm-6 col-md-4">
                        <x-form.date-picker id-start="start" id-end="end" name-start="start" name-end="end"
                            picker-type="date-range" :old="$filter->start" :old-end="$filter->end" />
                    </div>
                            ...
                            <tfoot>
                                <tr>
                                    ...
                                    <th class="text-end fw-bold">
                                        {{ number_format($cash + $bni + $bsi + $topup, 0, '', '.') }}</th>
                                </tr>
                            </tfoot>
                        </table>
                    </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            $("#btn-download-excel").click(function() {
                window.location =
                    `{{ route('finance.report.download.excel.payment-method') }}?start_date=${start_date}&end_date=${end_date}`
            })

            $("#btn-download-pdf").click(function() {
                window.location =
                    `{{ route('finance.report.download.pdf.payment-method') }}?start_date=${start_date}&end_date=${end_date}`
            })
        })
    </script>
@endpush
