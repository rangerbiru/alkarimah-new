            ...
            },
            {
                ...
                render: function(data, type, row) {
                    if (user_bendahara == "true") {
                        let url_verify = "{{ route('finance.transaction.verify.cash', 0) }}"
                        ...
                        let url_edit = "{{ route('finance.transaction.edit.cash', 0) }}"
                        let url_destroy = "{{ route('finance.transaction.destroy.cash', 0) }}"
                        let url_print = "{{ route('finance.transaction.print.cash', 0) }}"
                        ...
                        return `<a href="${url_print}" class="btn btn-info btn-xs set-tooltip" title="{{ __('label.print_proof_deposit') }}" target="_blank">
                                <i class="bx bxs-printer"></i>
                            </a>
                            <a href="${url_edit}" class="btn btn-dark btn-xs set-tooltip" title="${label_edit}">
                                <i class="bx bx-pencil"></i>
                            </a>
                            <a href="javascript:void(0)" class="btn btn-danger btn-xs set-tooltip" title="${label_delete}" onclick="deleteConfirm('${url_destroy}', false, 'table-deposit')">
