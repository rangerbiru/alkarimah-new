...
<body>
                ...
                @php
                    ...
                    foreach ($t->detail as $d) {
                        if (!array_key_exists($d->year->id, $transaction_year)) {
                            $transaction_year[$d->year->id] = [
                                'year' => $d->year->start . ' - ' . $d->year->end,
                                'list' => [],
                            ];
                        }

                        if (!array_key_exists($d->type->id, $transaction_year[$d->year->id]['list'])) {
                            $transaction_year[$d->year->id]['list'][$d->type->id] = [
                                'name' => $d->type->name,
                                'total' => 0,
                            ];
                        }
    ...
    <section class="page-break">
        ...
        <div style="margin-bottom: 15px;">Laporan Pembayaran Tanggal
            {{ Common::dateFormat($deposit->start_date) . ' s/d ' . Common::dateFormat($deposit->end_date) }}</div>
        ...
        <table class="table" style="margin-bottom: 20px;">
            ...
            @foreach ($transaction_year as $t)
                ...
                @foreach ($t['list'] as $l)
                    <tr>
                        @if ($index == 0)
                            <td rowspan="{{ count($t['list']) }}" style="text-align: center;vertical-align: top;">
                                {{ $t['year'] }}</td>
                        @endif

                        <td>{{ $l['name'] }}</td>
                        <td style="width: 25px;text-align: center;border-right: 0;">Rp.</td>
                        <td style="width: 125px;text-align: right;border-left: 0;">
                            {{ number_format($l['total'], 0, '', '.') }}</td>
                    </tr>
            ...
            <tr>
                ...
                <td class="bg-green" style="width: 125px;text-align: right;border-left: 0;">
        ...
        <table class="table" style="margin-bottom: 20px;">
            @foreach ($transaction_method as $t)
                <tr>
                    ...
                    <td style="width: 125px;text-align: right;border-left: 0;">
                        <b>{{ number_format($t['total'], 0, '', '.') }}</b></td>
                </tr>
            @endforeach
        </table>
