...
@section('title', $title)
@section('header')
    <x-section-page :label="$title" :icon="$icon" breadcrumb="finance/transaction/pending" />
@endsection
        ...
        <div class="card-body">
            <div class="row">
                <div class="col-sm-6 col-md-4">
                    <x-form.select id="type" :option="$types" :data-placeholder="__('label.all_transaction')" data-allow-clear="true" />
                </div>
                    ...
                    },
                    {
                        ...
                        render: (data, type, row, meta) => {
                            ...
                            ${row.number}
                                ...
                                html +=
                                    `<span class="fw-bold text-${row.flag_detail.color}">${row.number}</span>`
                            }
        ...
        function updateStatusPaid(id, status, unique_code) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    Swal.fire({
                        title: `<h5 class="mt-3">{{ __('label.unique_code') }}</h5>`,
                        html: `
                    <div style="text-align: left; font-size: 15px; margin-top: 10px;">
                        <p>Apakah Orang Tua melakukan Transfer beserta Kode Unik nya?</p>
                        <div class="form-check mb-2">
                            <input class="form-check-input swal-radio" type="radio" name="swal_unique_code" id="uc_yes" value="1" checked>
                            <label class="form-check-label" for="uc_yes">YA</label>
                        </div>
                        <div class="form-check mb-2">
                            <input class="form-check-input swal-radio" type="radio" name="swal_unique_code" id="uc_no" value="0">
                            <label class="form-check-label" for="uc_no">TIDAK</label>
                        </div>
                        <div class="form-check mb-2">
                            <input class="form-check-input swal-radio" type="radio" name="swal_unique_code" id="uc_custom" value="2">
                            <label class="form-check-label" for="uc_custom">SALAH / BERBEDA</label>
                        </div>
                        <div id="custom_code_container" style="display: none; margin-top: 15px;">
                            <label class="form-label" style="font-size: 13px;">Masukkan Kode Unik (1-3 digit angka terakhir):</label>
                            <input type="number" id="swal_custom_code" class="form-control" placeholder="Contoh: 123" min="0" max="999" oninput="if(this.value.length > 3) this.value = this.value.slice(0, 3);">
                        </div>
                    </div>
                `,
                        didOpen: () => {
                            const radios = Swal.getPopup().querySelectorAll('.swal-radio');
                            const container = Swal.getPopup().querySelector('#custom_code_container');
                            radios.forEach(radio => {
                                radio.addEventListener('change', (e) => {
                                    if (e.target.value === "2") {
                                        container.style.display = 'block';
                                        Swal.getPopup().querySelector(
                                            '#swal_custom_code').focus();
                                    } else {
                        ...
                        preConfirm: () => {
                            const selected = Swal.getPopup().querySelector('.swal-radio:checked').value;
                            const customCode = Swal.getPopup().querySelector('#swal_custom_code').value;

                            if (selected === "2" && !customCode) {
                                Swal.showValidationMessage(
                                    'Mohon masukkan angka kode unik yang ditransfer!');
                                return false;
                            }
                        ...
                        if (result.isConfirmed) {
                            const formData = {
                                id: id,
                                status: status,
                                unique_code_status: result.value.status_code,
                                custom_unique_code: result.value.custom_code
                            }
