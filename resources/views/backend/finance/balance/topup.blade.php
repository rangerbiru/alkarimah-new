...
@section('title', $title)
@section('header')
    <x-section-page :label="$title" :icon="$icon" breadcrumb="finance/balance/topup" />
@endsection
                ...
                <div class="card-body">
                    ...
                    <div id="start" class="text-center my-4">
                        ...
                        <h6 class="fw-normal text-muted mt-3" style="line-height: 23px;">
                            <b>{{ __('string.type_name_phone_to_search') }}</b><br />{{ __('string.search_deposit_info') }}
                        </h6>
                    </div>
...
@push('scripts')
    ...
    <script>
        ...
        function store() {
            ...
            $.ajax({
                type: "POST",
                url: "{{ route('finance.balance.store.cash') }}",
                data: formData,
                dataType: "json",
                success: function(response) {
