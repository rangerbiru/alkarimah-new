        ...
        <div class="card">
            <div class="card-body p-2">
                @if (empty($waiting))
                    ...
                    <x-form.input-group-mask id="nominal" mask="nominal" addon="Rp" :placeholder="__('label.input_nominal')" />
                        ...
                        <div class="me-2">
                            <div class="form-check form-check-md">
                                <input type="radio" name="method" value="{{ $method->bsi }}"
                                    class="form-check-input form-check-method rb-method-bsi" checked />
                            </div>
                        </div>
                        <div>
                            {{ __('label.bank_bsi') }}
                ...
                @else
                    ...
                    <div class="p-1">
                        ...
                        <div class="d-flex">
                            <div>
                                <span class="fw-bold text-grey">{{ __('label.transaction_number') }} :
                                    {{ $waiting->number }}</span>
                                <h3 class="text-success mb-0 mt-1">Rp. {{ number_format($waiting->total, 0, '', '.') }}
                                </h3>
                            </div>
                            <div class="ms-auto">
                                <img src="{{ $waiting->method->image_payment }}" class="img-thumbnail"
                                    style="height: 50px;" />
                            </div>
                        </div>

                        <div class="d-grid mt-2">
                            <a href="{{ route('finance.balance.waiting', $waiting->encrypted_id) }}"
            ...
            $("#btn-process").click(function() {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        btn.html("{{ __('label.process') }}").removeClass("btn-loader")
                            .removeAttr("disabled")
