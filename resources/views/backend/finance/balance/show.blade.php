    ...
    <div class="card-body p-3">
        <div class="d-flex">
            <div>
                ...
                <h3 class="{{ ($history->transaction->is_pengambilan_saldo) ? 'text-danger' : 'text-success' }} mb-0 mt-1">Rp. {{ number_format($history->transaction->total, 0, '', '.') }}</h3>
            </div>
            <div class="ms-auto">
        ...
        <div class="d-flex mb-4">
            <div>{{ ($history->transaction->is_pengambilan_saldo) ? __('label.withdrawal_date') : __('label.payment_date') }}</div>
        ...
        @if ($history->transaction->is_pengambilan_saldo)
            <div class="d-flex mb-2">
                <div>{{ __('label.withdrawal_amount') }}</div>
                <div class="ms-auto text-danger">-{{ number_format($history->credit, 0, '', '.') }}</div>
            </div>
        @else
            <div class="d-flex mb-2">
