...
<script>
    let currentPage = 1;
    let isPimpinan = {{ $isPimpinan ? 'true' : 'false' }};
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        // Hanya load positions jika user adalah pimpinan
        if (isPimpinan) {
            loadPositions();
        }
    ...
    function loadPositions() {
        fetch(`{{ route('employee.attendance-report.positions') }}`)
            .then(res => res.json())
            .then(data => {
    ...
    function loadData(page = 1) {
        ...
        const positionId = positionSelect ? positionSelect.value : null;
        ...
        fetch(`{{ route('employee.attendance-report.summary.data') }}?${params}`)
            .then(res => res.json())
    ...
    document.getElementById('export').addEventListener('click', () => {
        ...
        let url = `{{ route('employee.attendance-report.export') }}?month=${month}`;
        if (positionId) url += `&position_id=${positionId}`;
        url += `&is_pimpinan=${isPimpinan ? 1 : 0}`;
