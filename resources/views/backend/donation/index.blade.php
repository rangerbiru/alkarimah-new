...
<div class="card">
    <div class="card-body">
        <x-section-page-action
            ...
            :icon="$icon"
            :count="$count"
            :create-route="route('finance.donation.create')"
        />

        <div class="table-responsive">
            <table class="table" id="table-donation">
                <thead>
                    <tr>
                        <th>{{ __('label.no') }}</th>
                        <th>{{ __('label.name') }}</th>
...
$(document).ready(function() {
    ...
    window.LaravelDataTables["table-donation"] = $("#table-donation").DataTable({
        language: {
            search: "",
        ...
        ajax:
        {
            url: "{{ route('finance.donation.datatable') }}",
            type: "POST"
        },
        processing: true,
        serverSide: true,
        ...
        columns: [
            ...
            },
            {
                ...
                render: function(data, type, row) {
                    let url_edit = "{{ route('finance.donation.edit', 0) }}"
                    let url_destroy = "{{ route('finance.donation.destroy', 0) }}"
                    ...
                    return `<a href="${url_edit}" class="btn btn-dark btn-xs set-tooltip" title="${label_edit}">
                        ...
                        <a href="javascript:void(0)" class="btn btn-danger btn-xs set-tooltip" title="${label_delete}" onclick="deleteConfirm('${url_destroy}', false, 'table-donation')">
