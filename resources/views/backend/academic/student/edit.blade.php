...
@section('title', $title)
@section('header')
    <x-section-page :label="$title" :icon="$icon" breadcrumb="academic/student/edit" :breadcrumb-data="$student->encrypted_id" />
@endsection
        ...
        <div class="card-body p-1">
            <ul class="nav nav-tabs tab-style-1 d-sm-flex d-block" role="tablist">
                <li class="nav-item" role="presentation">
                    <a class="nav-link active" data-bs-toggle="tab" data-bs-target="#form-identity" aria-current="page"
                        href="#form-identity" aria-selected="true" role="tab">
                        <i class="fa-solid fa-address-card"></i> &nbsp;{{ __('label.identity') }}
                    </a>
                </li>
                <li class="nav-item" role="presentation">
                    <a class="nav-link" data-bs-toggle="tab" data-bs-target="#form-academic" href="#form-academic"
                        aria-selected="false" role="tab" tabindex="-1">
                        <i class="fa-solid fa-award"></i> &nbsp;{{ __('label.academic') }}
                    </a>
                </li>
                <li class="nav-item" role="presentation">
                    <a class="nav-link" data-bs-toggle="tab" data-bs-target="#form-excul" href="#form-excul"
                        aria-selected="false" role="tab" tabindex="-1">
                        <i class="fa-solid fa-universal-access"></i> &nbsp;{{ __('label.excul') }}
                    </a>
                </li>
                <li class="nav-item" role="presentation">
                    <a class="nav-link" data-bs-toggle="tab" data-bs-target="#form-other" href="#form-other"
                        aria-selected="false" role="tab" tabindex="-1">
            ...
            <form method="post" action="{{ route('academic.student.update', $student->encrypted_id) }}" class="form-block"
                ...
                <div class="tab-content">
                    <div class="tab-pane active border-0 pt-2" id="form-identity" role="tabpanel">
                        <div class="row">
                            <div class="col-sm-6 col-md-3">
                                <x-form.input-text name="nis" :label="__('label.nis')" :old="old('nis', $student->nis)" />
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-sm-6 col-md-3">
                                <x-form.input-text name="nik" :label="__('label.nik')" :old="old('nik', $student->nik)" optional />
                            </div>
                            <div class="col-sm-6 col-md-3">
                                <x-form.input-text name="nisn" :label="__('label.nisn')" :old="old('nisn', $student->nisn)" optional />
                            </div>
                            <div class="col-sm-6 col-md-3">
                                <x-form.input-text name="nis_local" :label="__('label.nis_local')" :old="old('nis_local', $student->nis_local)" optional />
                            </div>
                        </div>
                        <div class="row">
                        ...
                        <div class="row">
                            <div class="col-sm-6 col-md-3">
                                <x-form.input-text type="hidden" name="id_parent" id="parent-id" :old="old('id_parent', @$student->parent->encrypted_id)"
                                    readonly />
                                <x-form.input-group-button name="parent" id="parent" :label="__('label.parent')"
                                    :old="old('parent', @$student->parent->name)" button-id="btn-choose-parent"
                                    button-label="<i class='fa-solid fa-search'></i>" readonly />
                            </div>
                            <div class="col-sm-6 col-md-3">
                                <x-form.select name="religion" :label="__('label.religion')" :option="$religions" :old="old('religion', $student->religion->value)" />
                            </div>
                            <div class="col-sm-6 col-md-3">
                                <x-form.date-picker name="birthdate" picker-type="date" :label="__('label.birthdate')"
                                    :old="old('birthdate', $student->birthdate)" optional />
                            </div>
                            <div class="col-sm-6 col-md-3">
                                <x-form.input-text name="birthplace" :label="__('label.birthplace')" :old="old('birthplace', $student->birthplace)" optional />
                            </div>
                        </div>

                        <x-form.text-area name="address" :label="__('label.address')" :old="old('address', $student->address)" optional />
                    ...
                    <div class="tab-pane border-0 pt-2" id="form-academic" role="tabpanel">
                        <div class="row">
                            <div class="col-sm-6 col-md-3">
                                <x-form.input-text name="card_number" :label="__('label.student_card_number')" :old="old('card_number', $student->card_number)" optional />
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-sm-6 col-md-3">
                                <x-form.select name="id_asrama" :label="__('label.asrama')" :option="$asramas" :old="old('id_asrama', $student->id_asrama)"
                                    optional />
                            </div>
                            <div class="col-sm-6 col-md-3">
                                <x-form.select name="id_halaqah" :label="__('label.halaqah')" :option="$halaqahs" :old="old('id_halaqah', $student->id_halaqah)"
                                    optional />
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-sm-6 col-md-3">
                                <x-form.select id="education-level" name="level_education" :label="__('label.level_education')"
                                    :old="old('level_education', $student->class->level_education->value)" :option="$educations" />
                            </div>
                            <div class="col-sm-6 col-md-3">
                                <x-form.select id="class" name="id_class" :label="__('label.class')" :old="old('id_class', $student->id_class)"
                                    :option="[]" loading />
                            </div>
                            <div class="col-sm-6 col-md-4">
                                <x-form.input-text name="school_from" :label="__('label.school_from')" :old="old('school_from', $student->school_from)" optional />
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-sm-6 col-md-3">
                                <x-form.radio name="beasiswa" :label="__('label.scholarship')" :old="old('beasiswa', $student->beasiswa)" :option="$yesno" />
                            </div>
                        </div>
                    </div>
                    <div class="tab-pane border-0 pt-2" id="form-excul" role="tabpanel">
                        <x-form.select name="exculs[]" id="excul" :option="$exculs" :old="old('exculs')" multiple />
                    </div>
                        ...
                        <div class="row">
                            <div class="col-sm-6 col-md-4">
                                <x-form.input-file name="photo" id="photo" :label="__('label.photo')" accept-file="image"
                                    image-height="100px" :image-default="$student->file_photo" :info="__('string.info_photo') . '. ' . __('string.info_only_filled')" />
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-sm-6 col-md-3">
                            ...
                            <div class="col-sm-6 col-md-3">
                                <x-form.input-group-mask name="spp" :label="__('label.spp')" :old="old('spp', $student->spp)"
                                    mask="nominal" addon="Rp" optional />
                            </div>
                            <div class="col-md-6">
                                <x-form.input-text name="location" :label="__('label.location')" :old="old('location', $student->location)" optional />
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-sm-6 col-md-4">
                                <x-form.radio name="status" :label="__('label.status')" :old="old('status', $student->status)" :option="$activation" />
                            </div>
                        </div>
                    </div>
                </div>
            ...
            if (datatable_parent) {
                ...
                window.LaravelDataTables["table-parent"] = $("#table-parent").DataTable({
                    ...
                    columns: [{
                        ...
                        {
                            class: "align-middle",
                            render: (data, type, row, meta) => (row.id_relation == null) ? "-" : row.relation
                                .name
                        },
                        {
