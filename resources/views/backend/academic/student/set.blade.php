...
@section('content')
    <div class="card">
        <div class="card-body">
            <form id="form" method="post" action="{{ route('academic.student.store.set') }}" class="form-block">
                @csrf

                <div class="row">
                    <div class="col-sm-6 col-md-3">
                        <x-form.select id="asrama" name="id_asrama" :label="__('label.asrama')" :option="$asramas"
                            :old="old('id_asrama')" />
                    </div>
                </div>
                ...
                <div class="row">
                    ...
                    <div class="col-sm-6 col-md-3">
                        <x-form.select id="education-level" name="level_education" :label="__('label.level_education')" :old="old('level_education')"
                            :option="$educations" />
                    </div>
                    <div class="col-sm-6 col-md-3">
            ...
            $("#form").submit(function(e) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        setNotifSuccess(response.message,
                            "{{ route('academic.student.index') }}")
                    },
                    error: function(xhr, ajaxOptions, thrownError) {
                        ajaxLaravelError(xhr)
                        $("#form .btn-submit").html(
                            "<i class='fa-solid fa-paper-plane'></i> &nbsp;{{ __('label.save') }}"
                        ).removeClass("btn-loader").removeAttr("disabled")
                    }
                })
            })
        })
        ...
        function datatableStudent() {
            window.LaravelDataTables["table-student"] = $("#table-student").DataTable({
                ...
                columns: [{
                    ...
                    {
                        class: "align-middle",
                        render: (data, type, row, meta) => (row.class == null) ? "-" : htmlEntities(row.class
                            .name)
                    },
                    {
                        class: "align-middle",
                        render: (data, type, row, meta) => (row.asrama == null) ? "-" : htmlEntities(row.asrama
                            .name)
                    },
                    {
                        class: "align-middle",
                        render: (data, type, row, meta) => (row.halaqah == null) ? "-" : htmlEntities(row
                            .halaqah.name)
                    },
                    {
                        class: "align-middle",
