...
<x-section-page
    :label="$title"
    :icon="$icon"
    breadcrumb="academic/parent/create"
/>
@endsection
...
<div class="card">
    <div class="card-body">
        <form method="post" action="{{ route('academic.parent.store') }}" class="form-block">
            @csrf
...
let id_province = "{{ old('id_province') }}"
let id_city = "{{ old('id_city') }}"
let id_district = "{{ old('id_district') }}"
let id_village = "{{ old('id_village') }}"
