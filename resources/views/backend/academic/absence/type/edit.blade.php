...
<x-section-page
    :label="$title"
    :icon="$icon"
    breadcrumb="academic/absence/type/edit"
...
<div class="card">
    <div class="card-body">
        <form method="post" action="{{ route('academic.absence.type.update', $type->encrypted_id) }}" class="form-block">
            @csrf
            @method('PUT')
                ...
                <div class="col-md-3">
                    <div class="form-group">
                        ...
                        <div class="d-flex" style="border: 1px solid var(--input-border);padding: 0.44rem 0.75rem;border-radius: 0.35rem;">
                            <div><i class="{{ old('icon', $type->icon) }}" id="icon-class" style="font-size: 20px;"></i></div>
                            <div class="ps-2" id="icon-name">{{ old('icon', $type->icon) }}</div>
                            <div class="ms-auto">
...
$(document).ready(function() {
    ...
    $("#table-icon").on("click", ".btn-choose", function() {
        ...
        $("#icon").val(icon)
        $("#icon-class").removeClass("{{ old('icon', $type->icon) }}").addClass(icon)
        $("#icon-name").html(icon)
