...
@section('title', $title)
@section('header')
    <x-section-page :label="$title" :icon="$icon" breadcrumb="academic/basic" />
@endsection

@section('content')
    <div class="card">
        <div class="card-body">
            <x-section-page-action :label="$title" :icon="$icon" :count="$count" :create-route="route('academic.basic.create')" />
    ...
    <script>
        $(document).ready(function() {
            ...
            window.LaravelDataTables["table-class"] = $("#table-class").DataTable({
                ...
                ajax: {
                    url: "{{ route('academic.basic.datatable') }}",
                    type: "POST"
                },
                processing: true,
                serverSide: true,
                ...
                columns: [{
                    ...
                    {
                        class: "align-middle",
                        render: (data, type, row, meta) => htmlEntities(row.teacher)
                    },
                    {
                        class: "align-middle",
                        render: (data, type, row, meta) => htmlEntities(row.subject)
                    },
                    {
                        class: "align-middle",
                        render: (data, type, row, meta) => row.class
                    },
                    {
